@extends('layout.main')
@section('content')
    <h1 class="text-center" data-aos="fade-down">main page</h1>
    ...
    @if (Session::has('update'))
        <div class="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800"
            ...
            <div>
                <span class="font-medium">{{ Session::get('update') }}</span> Change a few things up and try submitting
                again.
            </div>
        </div>
    @endif
    <!-- Main modal -->
    <div id="addCustomer" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    <form class="space-y-6" action="{{ route('customer.create') }}" method="POST">
                        @csrf
                        @method('post')
                        <div class="mb-3">
                            <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your
                                name</label>
                            <input type="nama" name="customer_name" id="nama"
                        ...
                        <div>
                            <label for="countries"
                                ...
                                option</label>
                            <select id="countries" name="officer_id"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option selected>Choose officer</option>
                                @foreach ($customer as $item)
                                    <option value="{{ $item->officer_id }}">{{ $item->Officer->officer_name }}</option>
                                @endforeach
                            </select>
                        </div>
